...
- **fence.md 多消费者**：docs/design/fence.md（权威）+ editor/skill/references/fence.md（分发副本）+ editor/rules/*.tmpl（注入模板）+ v1-scope §2（引用）。改权威源后逐个同步。
...
- **围栏同步**：`filter`（标准 CSS 函数子集）+ `border-image-slice`（标准 CSS）进 v1-scope §2 支持列表（新增围栏属性触发 docs，design §4.4 补 filter 行）。`-l-slice` 保持砍（命名定 border-image-slice canonical）。
