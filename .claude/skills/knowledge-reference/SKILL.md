...
| 文档 | 性质 | 何时读 |
|---|---|---|
| `docs/design/00-main-design.md` | **设计权威契约**（当前实现真相源） | 查设计意图/契约（§4 围栏 §5 parse §6 Node §7 taffy §8 render §9 text） |
| `docs/design/fence.md` | 围栏属性权威（真相源 `fence_contract.rs`） | 查支持的标签/CSS/选择器 |
| `docs/roadmap/roadmap.md` | 范围 + 路线 + 机制草稿 | 查 v1 已交付/v1.x 路线/v1.x-v2 机制草稿 |
| `docs/superpowers/specs/` + `plans/` | 历史 spec/plan（开发过程记录） | 追溯某 feature 的设计/实现细节 |
...
├── samples/v1-showcase/      # showcase DSL 源（index.html+style.css+icons，引擎无关，Unity/Godot 共用）
...
### 2.3 scene（§6）
- **代际 NodeId**（v1.3+ 动态树）：`NodeId(pub u32)`，高 20bit index + 低 12bit gen + `INVALID=0xFFFF_FFFF`。对外 u32 不透明句柄（FFI/C# 零变化），删节点 gen++ 旧句柄自动失效。`scene.nodes: SlotMap<DefaultKey, Node>`（非 new_key_type!，因 slotmap KeyData 64bit 无法装 u32——见 §3.11 + 坑 96），`Scene::get(NodeId)/get_mut/key_for` 经 `NodeId::from_key/to_key` 桥接 DefaultKey。`id.index()`(=`.0>>12`) 用于每帧重建的并行数组（world_transforms/text_layouts，按 slot index 容忍空洞，长度 = `nodes.capacity()+1`）。
- `Node { id, parent, kind, base_style, style, taffy_id, layout_rect, clip_rect, children, dirty_mesh, dirty_text, classes, id_attr, touchable, hovered, active, disabled, draggable, tabindex, focused }`。**base_style（源，build/create/set_style 写）+ style（派生，每帧 rematch 从 base 起算 + 叠伪类）**——v1.3+ 显式分离（代码已落地，rematch_pseudo_classes 从 base_style.clone() 起算）。
- `NodeKind`: `Container` / `Button` / `Image{src}` / `Text{content}`。div/button 裸文本 → Text 子节点（§4.2，文本是 flex item）。
- `overflow:hidden` → `clip_rect = Some`（layout solve 后填实际框）。
- **动态树 API**（v1.3+，`scene/dynamic.rs`）：`create_node(kind,css)/create_root/append_child/insert_before/remove_child/set_text/set_src/set_style`（CSS 串入参复用 mapping::apply_decl）。`remove_node` 联动清 anim/scroll/tween/focused_node + slotmap remove gen++。详见 §2.29。
...
- v1 格式（扁平 + stringTable，LE）：Header 28B（magic `LPKG`=0x474B504C + version=1 + flags + nodeCount + stringCount + rootSizeX/Y）+ StringTable（u16 len+UTF8，**只** text content + image src）+ NodeBlock（每节点 parentIndex i32 + kind u8 + styleLen u32 + `bincode(ResolvedStyle)` blob + textIdx/srcIdx u16；NULL_IDX=0xFFFF；kind 0=Container/1=Button/2=Image/3=Text）。indexTable/压缩/分支推 formatVersion=3+（roadmap §5.6；v1b.3 已占 v2=AtlasSection）。
...
- **anim override = Scene transient 字段**（spec §4，关键）：`Scene.anim: AnimTable`（**v1.3+ 改 `HashMap<NodeId, NodeAnim>`**，同 `world_transforms` **不进 pkg**——旧 `Vec<NodeAnim>` 按 NodeId.0 索引在代际 NodeId 下越界，坑 97）。`NodeAnim{opacity/transform/bg_color/text_color}` 全 Option。**replace-override**：4 读取点 `unwrap_or(CSS)`——`compute_world_transforms` 读 `anim.transform.unwrap_or(style.transform.matrix)`（**不 compose，覆盖 css_matrix**）；`build_render_nodes` 读 `anim.opacity/bg_color/text_color.unwrap_or(style.*)`。`AnimTable::get` 经 `NodeAnim::is_empty` 过滤（全 None→None）→ 热路径退回 CSS **零回归**。一节点一 transform tween（Translate/Scale/Rotation 共享 transform 通道，并发 last-write-wins；混用嵌套 div）。持久：killed/自然完成停末值，`clear_anim(_prop)` 才回 CSS（= HashMap remove）。**颜色通道（bg_color/text_color）用 0-1 归一化**（mapping.rs:77 hex `/255.0`、render 测试 `[0,0,1,1]`=蓝；brief/草稿常写 0-255 致 clamp 全白——v1-showcase T8 implementer 查源修 `Rgba(/255f)`）。
...
- **tick 重排（§8，偏离 §12.7(2b')；v1.3+ T4 再调）**：`tween→focus→solve→refresh_content_sizes→process(仲裁+跟手)→wheel 消费+advance_all(惯性/回弹)→keys→compute_world_transforms→events→rematch→render`。**compute_world_transforms 从 solve 后移到 process 后**——读 scroll_pos（drag+inertia+wheel）同帧进 world matrix，**零拖拽延迟**。process 的 hit_test 用**上帧** world_transforms（1 帧差，§8.2 认：仲裁在 Down 未滚动前判定，clip viewport 固定主导）。**v1.3+ T4 删首帧 guard**（compute 改每帧 1 次 in process 后），hit_test 加 bounds guard（越界返 None=未命中，结构变更帧新增节点本帧不命中，1 帧延迟语义）。
...
### 2.24 v1e dirty hash + Unchanged emit（§614-623 blob 契约兑现）— 关 roadmap §1.5 性能基线
...
- **验收（双轨）**：criterion bench 500 节点——静态帧 476µs（全 Unchanged，比冷帧快 2.7× 证 dirty 生效）/ 冷帧 1.28ms / 换页帧 1.19ms，均 ≤2ms（roadmap §1.5 过线）。C# `_frameBuf` 改 ArrayPool（冷帧零 GC，ReadMesh per-node alloc 留观察撞墙再上）。家里机待验：PlayMode Profiler 静态帧≈0 upload + 冷/换页帧≤2ms + GC Alloc 静态帧≈0。
...
### 2.26 editor 工作流（v other，docs/superpowers/specs/2026-06-30-editor-workflow-design.md）
- **壳 = open-design 桌面 app**（nexu-io，不改源码在上面工作）。机制：`od project import <baseDir>` 导入目录为工作区 → daemon 把 project cwd = baseDir（`resolveProjectDir` usesExternalProjectRoot 分支，projects.ts:102）→ open-design spawn harness（Claude Code 等）在该 cwd → harness 自动读 cwd 的 `CLAUDE.md` + `.claude/skills/`（claude.ts:93 注释：Claude Code auto-loads project cwd 文件）。
- **LoomGUI editor 层**（`editor/` 模板源）：① `init.mjs`（Node 单文件，零依赖）交互输工作区/输出/harness → 拷围栏规则（CLAUDE.md 增量合并，标签 `<!-- loomgui-editor-begin/end -->` 包裹）+ skill 进目标工作区，pack.mjs 的 `__LOOMGUI_ROOT__` 占位符替换为仓库根（脚本目录上一层）。② `rules/{claude,opencode,codex}/*.tmpl` 三 harness 规则模板。③ `skill/loomgui-editor/`（SKILL.md + tools/pack.mjs 封装 loomgui_pkg + references 副本）。
- **pack.mjs 封装 loomgui_pkg**（不暴露 Rust）：定位仓库根 → `cargo build -p loomgui_pkg --release`（needBuild：bin 不存在或 main.rs/lib.rs/Cargo.toml mtime 新）→ 调 CLI 透传 exit code/stderr。验证器即打包器（loomgui_pkg 打包期 FENCE_TAGS 围栏验证，违规非零退出，AI 据此自纠）。
- **samples/ 合并 editor-test**：`samples/` 自身当被注入的测试工作区（init 注入 CLAUDE.md/.claude/，gitignore 生成物）+ v1-showcase 基线 + `design-systems/loomgui/` 测试夹具（暗色 dashboard，仅测试用，不进 init 注入物）+ ai-output。
- **B 方案不提供正式 design-system**：每个 UI 风格由设计师 prompt 驱动，design-system 仅测试夹具。围栏规则不进 DESIGN.md（放 CLAUDE.md，harness 自动读）。
- **严 polyfill 固化**（坑 95）：`editor/skill/loomgui-editor/references/preview-polyfill.html` 标准片段（`div{display:flex;flex-direction:column}` + `*{box-sizing:border-box}` + `body{margin:0}`），SKILL.md 工作流强制 AI 生成时内联进 head。LoomGUI 契约 div 永远 flex column，Chromium 默认 div=block → 不挂 polyfill 预览骗 AI。polyfill 在 head `<style>`（预览用），设计师样式放外部 css（打包用），pack.mjs 吃外部 css、parse_html 忽略 head（dom.rs:34），polyfill 不进 pkg。
- **l-container 砍**（坑 94）：FENCE_TAGS 白名单只留 div/span/img/button（全 HTML 标准）。l-container 与 div 同映射是冗余假自定义元素，Chromium 不认预览塌 + AI 困惑。`l-` 前缀留给 l-list/l-rich 等 v1.x 真·自定义元素。tag→NodeKind 映射在动态树重构后提到 `scene/dynamic.rs::kind_from_tag`（runtime 建树 API 复用），parse 路径 `gather_rec` 调它。

### 2.27 围栏契约 + 维护机制（docs/design/fence.md）
- **单一真相源 = `loomgui_core/tests/fence_contract.rs`**（可执行围栏契约测试）。`docs/design/fence.md` 是人类可读副本，不一致时测试赢。防漂移门：`cargo test -p loomgui_core fence_contract`（build .dll 前 / 改 apply_decl/FENCE_TAGS/selector 后必跑）。
- **测试三类断言**：A 元素围栏（FENCE_TAGS 白名单接受 / 围栏外报错）；B 支持属性（apply_decl 返回 true + 关键项字段断言，如 display:grid 落 Display::Flex）；C 围栏外属性静默忽略（apply_decl 返回 false + 字段不变，如 position:absolute 不脱离流保持 Position::Relative）。
- **改围栏流程**：加 apply_decl arm → 加 fence_contract 断言 → 同步 fence.md 副本（坑 83：忘同步分发副本）。
- **fence.md 多消费者**：docs/design/fence.md（权威）+ editor/skill/references/fence.md（分发副本）+ editor/rules/*.tmpl（注入模板）+ roadmap §1.2（引用）。改权威源后逐个同步。

### 2.28 ColorFilter + 九宫格 slice（v1.3，§8.2 MeshFactory + §4.4 围栏）
- **ColorFilter**（标准 CSS `filter` → 4×5 矩阵，照搬 fgui `ColorFilter.cs`）：`style/color_filter.rs` 提供 `IDENTITY`/`grayscale/brightness/contrast/saturate/hue_rotate/invert` 预设 + `concat(a,b)=a×b` 矩阵相乘（行主序 20 float，alpha 行恒 (0,0,0,1,0) → alpha 不变）。`mapping.rs::parse_filter` 解析 `filter:grayscale(1) brightness(1.2)` 函数链 → 矩阵相乘（**新 preset 左乘** `concat(&m,&acc)` 匹配 fgui `ConcatValues`，坑 85）。`ResolvedStyle.color_filter: Option<[f32;20]>`（None=无 filter）。**sepia 暂用 grayscale 占位**（Tint 矩阵 defer，spec §9）。
- **ColorFilter 管线**：有 filter → `program=3`（filter+tex*vcol base，Image+filter / Container+filter-无bg-image）或 `program=4`（filter+BG_COMPOSITE base，Container+bg-image+filter，spec §3.2 双 keyword，坑 84）。矩阵进 **frame blob SOA `color_matrix` 列（[f32;20]，80B/节点，第 20 列，VERSION 5→6）**——frame blob 是 Rust→Unity 唯一通道，矩阵必须进 blob 不能只 MPB（坑 87）。Unity 侧 `MirrorPool::SetColorFilterMatrix` 读 `blob.ColorMatrix(i)` 拆 5 Vector MPB SetVector（`_CF0..3`+`_CFOff`，照搬 fgui UpdateMatrix，同 `_ObjM` MPB 模式）。shader `#pragma multi_compile _ COLOR_FILTER` + frag 后处理 `col.rgb=mul(cfM,float4(col.rgb,1)).rgb+_CFOff.rgb`（alpha 不变，放 color 分支后 CLIPPED 前，可与 BG_COMPOSITE 叠加）。Material key 不加 filter 维度（program=3/4 已独立，矩阵 MPB per-renderer 不拆 Material）。
- **九宫格**（标准 CSS `border-image-slice`，照搬 fgui `Image.cs:286 SliceFill`）：`mapping.rs::parse_slice` 解 4 值上右下左（px 存像素，% 存比例渲染期 resolve）。`ResolvedStyle.border_image_slice: Option<SliceInsets{top,right,bottom,left}>`。`mesh::nine_slice(rect,color,slice,src_w,src_h,uv_min,uv_max)` 产 4×4 顶点（gridX/gridY 各 4 值）+ 9 quad（`TRIANGLES_9_GRID` 固定索引）+ clamp 防 rect 比源图小时四角越界。四角不缩放/四边单轴拉伸/中心双轴拉伸。`TextureRegion.width/height` 是 u32，nine_slice 要 f32 → `as f32`。
- **slice+radius 共存**（`mesh::nine_slice_rounded`，fgui 无现成自设计）：four-mode 分流 `(has_slice, all_zero)` → quad/rounded_rect/nine_slice/nine_slice_rounded。nine_slice_rounded = 四角纯弧扇形（几何圆角，mirror rounded_rect，弧外不绘制）+ L 形 quads 覆盖 `[r,slice]` 间隙 + 边/中心切片拉伸。不变量：四角不拉伸（UV 1:1 源角区）、四边单轴、中心双轴、radius=0 退化 nine_slice。**坑 86**：初版角扇形覆盖 `[0,r]²` + strip 从 slice 起 → `[r,slice]` 12% 洞 + 圆角靠纹理 alpha 伪造；fix 重设计 + 覆盖率测试（point-in-tri 扫描，arc-cutout 点 (2,2) 须不覆盖）。
- **dirty hash**：Mesh arm 加 `program.hash` + `color_matrix` 全量 hash（捕 filter 开关 + 矩阵变，坑 84 延伸——program 3↔4 切换也要捕）。九宫格 mesh >4 顶点走 `verts.len()>4` 全顶点分支（v1.2 坑 76 就绪）。PKG_FORMAT_VERSION 9→10（color_filter + border_image_slice 两字段进 ResolvedStyle）。
- **围栏同步**：`filter`（标准 CSS 函数子集）+ `border-image-slice`（标准 CSS）进 roadmap §1.2 支持列表（新增围栏属性触发 docs，design §4 补 filter 行）。`-l-slice` 保持砍（命名定 border-image-slice canonical）。
- **v1.3 家里机 PlayMode 验收续修**（运行时 bug 链，公司机静态测试 + final review 全漏，坑 88-93）：① filter MPB 漏 program=4（坑 88，MirrorPool `==3`→`==3||==4`）② BG_COMPOSITE 合成 alpha 改 source-over（坑 89，坑 79 方案 A `col.a=vcol.a` 无底色丢图→`col.a=tex.a+vcol.a·(1-tex.a)`）③ brightness 改 CSS 乘法（坑 90，fgui AdjustBrightness 加法不符 CSS `rgb×n`）④ filter 矩阵在 sRGB 应用（坑 91，shader linear→sRGB→矩阵→sRGB→linear）⑤ nine_slice_rounded UV 按 slice 分段（坑 92，右角区钉 umax 不超 atlas）⑥ **打包器 uv v flip**（坑 93，asset/mod.rs PNG y-down→Unity y-up，atlas 加 108px skin.png 打破"占满高"巧合暴露）。**background-position**：fit_uv cover 改取子区顶（图顶对齐容器顶，CSS `0% 0%` 默认）+ contain geometry 缩小左上子矩形（UV 整子区，留白右下）——之前硬编码居中（cv=0.5）不符 CSS 默认。showcase §3.7/3.9 换 skin.png（FairyGUI Bag/2.png 九宫格皮肤，108×108）+ 重打 pkg.bin。**dump_bg.rs** 诊断 example 建立必用 pkg.bin 路径（§6）。

### 2.29 动态树重构（v1.3+，§6 scene + §14 FFI）— static-tree → dynamic-tree
- **动机**：v1 static-tree（pkg 加载即冻结）能撑 showcase 静态样张，但 v1.4 列表/v1.5 Controller/v1.6 富文本/v1.7 IME 全需运行时改树，v1.4 第一个撞墙。v1.3+ 是插在 v1.4 前的地基重构（非功能号），把树解冻。设计合理 > 工作量（个人兴趣项目，代际 NodeId 选最干净非最小）。
- **代际 NodeId**（`scene/node.rs`）：`NodeId(pub u32)` = 高 20bit index + 低 12bit gen + `INVALID=0xFFFF_FFFF`。对外 u32 不透明句柄（FFI/C#/FrameBlob/pkg.bin 零变化），删节点 gen++ 旧句柄自动失效（slotmap.get 返 None）。`index()`=`.0>>12`（slot 槽位号，并行数组用），`gen()`=`.0&0xFFF`。
- **slotmap 存储**（`SlotMap<DefaultKey, Node>`）：spec 原定 `new_key_type!` 不可行——slotmap 1.1 KeyData 是 `{idx:u32, version:NonZeroU32}` 64bit 私有字段，无法装 u32 FFI 句柄（坑 96）。改 DefaultKey 桥接：`NodeId::from_key(k)`=`(idx<<12)|(version&0xFFF)`，`to_key()` 经 `KeyData::from_ffi((version<<32)|idx)` 重构（slotmap version 恒奇→12bit 截断仍奇→from_ffi no-op→round-trip 一致，reviewer 核代际安全成立）。`Scene::get/get_mut/key_for` 经桥接。slotmap idx **从 1 起**（idx 0 是 sentinel slot）→ 首节点 NodeId.0=4097 非 0，测试用 `roots[0]`/`find_node_by_id` 派生 id 不硬编码。
- **持久附属 HashMap**（anim/scroll）：spec 原定 SecondaryMap 不可行——slotmap `Key` 是 unsafe trait（非 sealed）+ KeyData 64bit 与 NodeId 32bit 不匹配，手 unsafe impl 违反设计意图（坑 96）。改 `HashMap<NodeId, T>`（NodeId 已 Hash+Eq），零转换零 trait 限制，悬空安全（删节点联动 remove）。查询 O(1) HashMap，u32 hash 快，节点数千量级可接受。
- **每帧重建并行数组**（world_transforms/text_layouts/taffy_ids）：保持 `Vec<T>`，长度 = `nodes.capacity()+1`（slotmap idx 从 1 起 + 删留洞 capacity≥max idx），按 `id.index()` 索引，空洞 IDENTITY/None 无害。**T5 根因修复**（坑 98）：原按 `nodes.len()` 分配，remove 后 slotmap 间隙致高 idx live 节点越界 panic，改 capacity+1。
- **动态 API**（`scene/dynamic.rs` + Stage 转调 + 9 FFI）：`create_root/create_node(kind,css)`（CSS 串复用 `mapping::apply_decl`，runtime 可用不依赖 parse feature）、`append_child/insert_before(ref=INVALID 末尾追加)/remove_child`（child 无父检查）、`remove_node`（递归删子 + 从父/roots 摘 + 联动清 anim/scroll/tween/**focused_node** + slotmap remove gen++）、`set_text/set_src`（改 NodeKind + dirty）、`set_style`（改 base_style + dirty，下帧 rematch 从 base 重算 style）。
- **remove_node 联动清不变量**（同构坑 88 教训）：删节点 = 全持久附属同步清事件——anim.clear_node + scroll.remove + tweens.kill_node + focused_node 清（final review 补）。新持久附属加入时必须同步加 remove_node 联动。`TweenManager::kill_node` 标 killed，update 双保险（killed 标志 + `scene.get(t.node).is_none()` live 校验，代际化后比旧 idx 边界强）。
- **world_transforms 时机前移**（T4）：compute 从"末尾+首帧 guard"改"process/scroll 后、render 前，每帧 1 次"。删首帧 guard。hit_test 加 bounds guard（越界返 None=未命中）。scroll_pos 同帧进 world matrix 保留，hit_test 1 帧延迟保留。
- **base/style 分离**：代码已落地（`Node.base_style` 源 + `Node.style` 派生，`rematch_pseudo_classes` 从 `base_style.clone()` 起算）。set_style 改 base_style，下帧 rematch 自动重算 style。动态建节点 base_style = create_node CSS parse 结果。
- **FFI/C# 透明**：9 新 FFI（create_root/create_node/append_child/insert_before/remove_child/remove_node/set_text/set_src/set_style），NodeId 仍是 u32，C# 侧 `uint` 句柄。csbindgen 生成 9 DllImport。LoomStage 加 `LoadPackageFile(pkgFile)`（运行时切 pkg + 重 LoadAtlas + 清 MirrorPool）+ EventHandler.Clear（切 pkg 清旧 listener）。
- **showcase §3.10 验收**（四用例）：建/删子树 + set_style toggle + 批量建 20 + 动态加载 bin（切 mail pkg/showcase pkg，切回重 SubscribeAll）。spec `docs/superpowers/specs/2026-07-01-v1.3-dynamic-tree-design.md`、plan `docs/superpowers/plans/2026-07-01-v1.3-dynamic-tree.md`。踩坑：slotmap KeyData 64bit（坑 96）、AnimTable 读写不对称（坑 97）、并行数组 len 分配越界（坑 98）、remove_node 漏清 focused_node（坑 99）。
- **已知潜伏债**（非阻塞）：① flag-pack sentinel（thumb `container.0|V/H_THUMB_FLAG` bit 29/30 与 20bit index 编码理论重叠，slotmap idx≥262144 才爆，实际节点数千不撞）② version 12bit 截断（同槽复用 ~2048 次后 wrap，安全返 None 非错位）。
...
### 3.11 slotmap 1.1.1（scene/node.rs，v1.3+ 动态树）
...
2. **div 永远是 flex 容器，只装 flex item**。无浏览器 block/inline flow。**围栏不暴露 `<l-rich>`/`<l-list>`**——富文本/虚拟列表由代码层做，AI 不知道就不会写（design §4.1）；元素内"文本+元素+文本"行内混排报错。
3. **div/button 裸文本 → Text 子节点**（§4.2）。`<div>标题</div>` 产出 Container + Text 子「标题」（文本是 flex item），**不丢弃**。
4. **行内混排报错**（文本+元素同在）。解析期 Err，提示用 span 拆块级（围栏不暴露 l-rich）。
5. **围栏外元素报错不降级**。parse 白名单 `[div/span/img/button]`（l-container 砍，坑 94），其它 Err。
...
**症状**：§3.6 `.bg-demo` 在浏览器直接打开，`contain` 行（983×64 容器装 64×64 home.png）平铺出 ~15 个图标，非预期单图居中。
...
**症状**：§3.6 浏览器打开，cover/100% 行"看不到图"——图在画但被横向拉到 983 宽 + 纵向裁切，线条 icon 退化成认不出的横带；contain 图又只占 6.5% 宽太小。
...
**症状**：v1.3 spec §3.3 写"矩阵走 MPB"、§5 写"blob 零改"，但 MPB 是 Unity 侧，矩阵值在 Rust 侧——Unity MPB 无值可 Set。
...
**症状**：§3.8 cf-demo 全青色，filter 不变色（仅隐见房子轮廓）。
...
**症状**：§3.6 第4行 eye.png 无底色、§3.7 图+圆角、§3.9 图全消失。
...
**症状**：§3.8 brightness(1.5) 发白非提亮，和 html 对不上。
...
**症状**：§3.8 saturate/hue/contrast 色相和 html 对不上。
...
**症状**：§1.2/1.3 img + §3.6 bg-image 图上偏（design 顶采到图底）。
...
**根因**：LoomGUI 契约 div 永远 flex column（00-main-design §109），Chromium 默认 div=block/content-box/body 8px margin → 不挂 polyfill 预览骗 AI（gap/flex-grow/align-items 全不生效）。polyfill 靠手抄易漏。
...
- **dump_bg.rs**（v1.3 验收诊断 example）：`load_package(pkg.bin)` 遍历 Mesh 节点 dump rect/program/vcol/uv 区间/uv0(design 顶点)/color_matrix/verts，定位偏移/滤镜色差/slice 失真/UV 超界。**必用 pkg.bin 路径**。
- **inline vs pkg.bin 路径**（坑）：dump 用 `load_inline` 时 TextureRegistry 空（tex=0 哨兵），bg-image 走 program=0 非 2/4、UV 全图不代表实机；要看真实 program/uv 必 `load_package`（和 Unity 同源 atlas）。
- **atlas analyze_image**（v flip 诊断，坑 93）：loom_showcase.atlas.png 上传看布局（单行/多行、icon 顶/底、正立/颠倒）+ dump 的 uv v 区间对照，定位 v 约定错（打包器 PNG y-down vs Unity y-up）。
...
- **showcase 源在 `samples/v1-showcase/`**（引擎无关，`c3cc5bc` 挪出 Unity）：HTML/CSS/icons 是 LoomGUI 的 DSL 源，Unity/Godot 共用，放顶层 `samples/` 不放 `loomgui_unity/`。Unity 实读 `StreamingAssets/loom_showcase.pkg.bin`（showcase 目录里**不放 pkg.bin/atlas.png 产物副本**——那是旧的，Unity 不读）。改 showcase HTML 后重打：`cargo run -p loomgui_pkg -- samples/v1-showcase/index.html samples/v1-showcase/style.css -o loomgui_unity/Assets/StreamingAssets/loom_showcase.pkg.bin -a loom_showcase.atlas.png -w 1080 -h 1920`。`dump_render.rs` 读 `samples/v1-showcase/`（路径已改）。
- **open-design 调研方法**（v other 壳选型）：open-design 源码在 `temp/open-design/`（只读参考，gitignore /temp/）。关键扩展点：① design-system = `design-systems/<slug>/` 目录（DESIGN.md+tokens.css+components.html+manifest.json，丢文件夹即进 picker）；② skill = `skills/<name>/SKILL.md`（frontmatter name/description/triggers + od: 块）；③ `lint-artifact.ts` 是硬编码函数**非扩展点**（roadmap §2.2 原说加扩展点 = 错，砍掉 grep linter 层）；④ 打包 app 用户扩展目录 `AppData\Roaming\Open Design\namespaces\release-stable-win\data\{design-systems,skills}\`（USER_DESIGN_SYSTEMS_DIR/USER_SKILLS_DIR，server.ts:823-824）；⑤ `od project import <baseDir>` 导入目录为工作区，project cwd = baseDir（projects.ts:102 usesExternalProjectRoot），harness 在该 cwd 启动自动读 CLAUDE.md/.claude（claude.ts:93）。
- **subagent-driven 假阴性测试**（坑 82 教训）：SDD task reviewer 审测试时，若测试只断言"结构"（标签计数/文件存在/返回值）不查"内容"（旧内容是否真被替换/字段是否真变化），会漏过实现 bug。controller 收到 review clean 不能全信——final whole-branch review（opus 跨 task 视角）才能抓到单 task review 漏的（Task 4 review 漏了正则 bug，final review 抓到）。增量合并/替换类逻辑的测试**必须断言"旧内容不残留"**。
- **mesh 覆盖率 point-in-tri 扫描**（坑 86 教训）：自设计 mesh（fgui 无参考，如 nine_slice_rounded）的覆盖率验证不能只数顶点/UV——写测试 helper 遍历 mesh 三角形（`idx.chunks(3)` + 叉积符号判 point-in-tri），在网格采样点断言"该覆盖的覆盖、该镂空的镂空"。圆角/异形 mesh 的镂空区（弧外、圆角切口）须有"该点不覆盖"断言，否则 implementer 画满靠纹理 alpha 伪造能过顶点数测试。比"数顶点"严，是自设计几何的正确性门。
- **final review 跨 task 数据流追踪**（坑 84/87 教训）：v1.3 final review 抓 2 个 per-task 漏的 cross-task bug（program 语义接力裂缝 + spec 数据流不完整）。方法：opus reviewer 追"值从产生到消费的完整跨 task 链"（如 ColorFilter：CSS→mapping→ResolvedStyle→render program→blob 列→FrameBlob.cs→MirrorPool MPB→shader），逐环节验无断链。per-task reviewer 只看自己 task 内部一致，看不到接力裂缝——final review 的价值在此。
- **worktree 隔离开发**（main 有未提交中间态时）：main 直推工作流下，若 main 工作区有别的 session 的未提交改动（如动态树重构中间态），新 feature 用 `EnterWorktree` 从 origin/main 分出隔离 worktree 跑，不碰主工作区。worktree 默认从 `origin/main`（fresh）分，**不是本地 main HEAD**——本地 main 未 push 的 commit（含 spec/plan）要 cherry-pick 进 worktree。完成后 rebase 到本地 main HEAD（解冲突）+ fast-forward 合并。rebase 解冲突注意：被重构移走的逻辑（如 tag→NodeKind 提到 kind_from_tag）里的清理易漏，需全仓库 grep 验证。worktree 清理用 `ExitWorktree`（target/ 编译产物锁可能 permission denied，注册清了即可，目录残留不影响 git）。
...
**v1d 路线固化（main @ ea38bd5）**：v1d = 各轮 spec 标 v1d/v1d+ 全量项照单全收（治 defer-defer 遗失），12 项 5 子轮：v1d.1 拖拽+长按+safe-area / v1d.2 键盘+焦点 / v1d.3 transform / v1d.4 GTween / v1d.5 ScrollPane+滚轮+手势仲裁。仅 v1d.5←v1d.1 依赖；IME 默认 defer 随 TextInput（v1.x）；broadcast/AncestorChain/invalidation 不进 v1d（v1.x/v1e）。关 v1 验收 #2 可滚动容器 + #4 safe-area。明细已并入 roadmap §1，原 `v1d-plan.md` 已删（git history 保留）。
...
**v1.2 border-radius（main @ 2a467f2，spec `2026-06-29-v1.2-border-radius-design.md`）**：批 1 第 2 项完成。SDD 9 task + 4 fix（spec→plan→9 task→opus final review）。`render/mesh.rs::rounded_rect` 三角扇圆角（照搬 fgui `RoundedRectMesh` 自适应分段/末段精度锁/直角分支 + 两改进：CSS 按边缩放钳制、TL→TR→BR→BL 角序），Container/Button 分支 `%` 渲染期 resolve + all_zero 走 quad 零回归。支持 1~4 值 + `h/v` 椭圆角 + px/%。PKG_FORMAT_VERSION 8→9（坑 74）。showcase §3.7 卡。**final review 抓 I1**（坑 76：dirty hash 采样首末顶点漏圆角 mesh BL-only 变 → >4 顶点 hash 全顶点）。core 385 测全绿。**家里机 PlayMode 验收抓 3 运行时 bug**（本机静态/单测全漏）：坑 77（混合椭圆角直角分支落圆心+方向偏离矩形顶点，9e60302 修）+ 坑 78（scroll+CLIPPED clip rect design/world 空间错位全裁，a34c972 v1 错→e2b64bd v2 修）+ 坑 79（shader tex×vcol 非 CSS 合成，图透明区全透明不透 bg-color，v1.1 spec §6.2 承诺错误）。**圆角毛刺 /4 加密**（`6ad13a6`）：`rounded_rect` 分段 `ceil(π·r/8)+1`→`/4`（r=8 25→37 顶点）消实机圆角锯齿；pkg.bin 不变（mesh 运行时算）。

**坑 79 bg-image 共存视觉修复（方案 A 已实现，main @ 0bf2b46，spec `2026-06-30-bg-image-composite-design.md`）**：inline 5 task（spec→plan→TDD）。blob 加 program 列（u8，第 19 列，VERSION 4→5，`*program as u8`）+ FrameBlob.cs `Program(i)` + header offset 18→19；core Container+bg-image 命中纹理（tex_id≠0）→ program=2（字段 v1.2 已埋，只改赋值）；shader `#pragma multi_compile _ BG_COMPOSITE` + `#elif` 合成分支 `col.rgb=tex.rgb×tex.a+vcol.rgb×(1-tex.a); col.a=vcol.a`（linear 空间正确）；MaterialManager `program==2` EnableKeyword；MirrorPool 用 `blob.Program(i)`。pkg.bin 零改（frame blob 字段非 scene 字段）。core 448 测全绿。**待家里机 PlayMode 验收** §3.6（图透明区显 bg-color）+ §3.7 R5（图+圆角共存）。**注意**：Container+bg-image 无 bg-color 时 vcol=透明 → 合成整块透明（既有行为，非本坑引入，§6.2 承诺只对有 bg-color 成立）。

**showcase 源挪顶层 samples/（`c3cc5bc`）**：`samples/v1-showcase/`（index.html+style.css+icons，引擎无关，Unity/Godot 共用）。删旧产物副本 + Unity meta + 空 Samples 目录。dump_render.rs 路径改。pkg.bin 不重打（内容未改）。详见 §6 showcase 源技巧。

**坑 80 showcase 缺 no-repeat fix**：`.bg-demo`/`.br-demo` 加 `background-repeat:no-repeat`——浏览器默认 repeat 致 contain 行平铺，与 Unity 单图不一致。LoomGUI 不支持此属性（忽略，Unity 无影响）。pkg.bin 不重打（纯 CSS 文本属性，LoomGUI 不认，不影响 scene）。详见坑 80。

**坑 81 showcase bg-demo 容器尺寸 fix**：`.bg-demo` 983×64 极端扁宽 → 160×120（4:3）。旧尺寸致 cover/100% 把 64×64 图拉成认不出横带、contain 图太小，size 三模式差异退化。**pkg.bin 重打**（容器尺寸是 scene 布局字段，非围栏外属性，与坑 80 no-repeat 不同）。详见坑 81。

**v other editor 工作流完成**（`a23274a..6434938`，11 commits）：editor/ 模板源（init.mjs 注入脚本 + rules 三 harness 模板 + skill 封装 loomgui_pkg）+ `loomgui_core/tests/fence_contract.rs` 围栏契约测试（真相源，10 tests）+ `docs/design/fence.md` 围栏权威副本 + `samples/design-systems/loomgui/` 测试夹具。SDD 8 任务 + final review（C1 正则 bug + I1 fence.md 副本漂移已修，坑 82/83）。**待验收**：open-design import samples/ + 家里机 Unity 加载 ai-output/pkg.bin。机制详见 §2.26/2.27，open-design 调研详见 §6。

**fence-cleanup 完成**（`749e478..eb6041c`，8 commits，worktree `worktree-fence-cleanup` rebase 合并 main）：editor 工作流实测 backpack.html 预览塌暴露两个问题。① **砍 l-container**（坑 94）：FENCE_TAGS 白名单只留 div/span/img/button，l-container 与 div 同映射是冗余假自定义元素，Chromium 不认预览塌 + AI 困惑。同步 node.rs 映射 + dynamic.rs `kind_from_tag`（动态树重构提取的，rebase 解冲突补）+ 围栏/设计文档（fence.md/00-main-design/roadmap/editor 副本/rules 模板）+ backpack.html。② **严 polyfill 固化**（坑 95）：`editor/skill/loomgui-editor/references/preview-polyfill.html` 标准片段（div flex column + box-sizing + body margin:0）+ SKILL.md 强制 head 内联 + preview-trust.md 补（div 需 polyfill + overflow:scroll 真滚动可信）。SDD 5 task + final review clean。rebase 解 2 处 node.rs 冲突（动态树把 tag→NodeKind 提到 kind_from_tag，fence-cleanup 的 inline match 被取代）。**worktree 隔离开发**：main 有动态树中间态，用 EnterWorktree 从 origin/main 分出跑，rebase 到本地 main HEAD + fast-forward 合并。机制详见 §2.26 polyfill/l-container 段，坑 94/95，worktree 技巧详见 §6。

**v1.3 ColorFilter + 九宫格 slice 完成**（`32d54a6..cf6b00e` + fix `2563ba4`，spec `2026-06-30-v1.3-colorfilter-nineslice-design.md`）：批 2 第 3 项。SDD 11 task + final review（opus 抓 2 cross-task bug，per-task 全漏）。**ColorFilter**：标准 CSS `filter` 函数（grayscale/brightness/contrast/saturate/hue-rotate/invert/sepia 占位）→ 4×5 矩阵（照搬 fgui ColorFilter.cs），program=3/4 分流（坑 84），矩阵进 frame blob `color_matrix` 列（v6，坑 87），Unity MPB SetVector ×5 + shader COLOR_FILTER 后处理。**九宫格**：`border-image-slice`（标准 CSS）→ `mesh::nine_slice`（照搬 fgui SliceFill 16 顶点）+ `mesh::nine_slice_rounded`（自设计 slice+radius 共存，坑 86 fix：纯弧扇形 + L 形 quads）。**编号统一**：roadmap v1.x 7 号方案（9 项合并，v1.3=ColorFilter+九宫格+profiling）。PKG_FORMAT_VERSION 9→10（color_filter + border_image_slice 字段）；blob VERSION 5→6（color_matrix 列）。围栏同步：filter + border-image-slice 进 roadmap §1.2 + design §4。core workspace 416 测全绿（+I1/I2 fix 2 测 + fence_contract 10 测）。showcase §3.8（7 filter）+ §3.9（3 nine-slice 含 slice+radius）。**家里机 PlayMode 验收续修（坑 88-93，公司机静态测试 + final review 全漏）**：§3.8 filter 全青色（坑 88 MirrorPool MPB 漏 program=4）→ 修；§3.6/3.7/3.9 图消失（坑 89 合成 alpha source-over，坑 79 方案 A 修正）→ 修；§3.8 色差（坑 90 brightness 改 CSS 乘法 + 坑 91 filter 在 sRGB 应用）→ 修；§3.9 slice+radius 失真（坑 92 nine_slice_rounded UV 分段）→ 修；§1.2/1.3 img + §3.6 bg-image 图上偏（坑 93 打包器 uv v flip，atlas 加 108px skin.png 打破占满高巧合暴露）→ 修。background-position cover 取子区顶 + contain geometry 缩小左上（对齐 CSS 0% 0%）。showcase §3.7/3.9 换 skin.png（FairyGUI Bag 2.png 九宫格皮肤 108×108）+ 重打 pkg.bin。core 417 测全绿。**待验**：v flip 修复后 img/bg-image 正向（dump 验 design 顶→uv v=1.0=texture 顶，Unity 重启验）。机制详见 §2.28，坑 84-93。`2563ba4` commit message 误导（说 fence_contract 实际含 I1/I2 fix，用户手动 commit 混入），代码正确未 push 时可 amend。

**v1.x 编号统一**（`5937146`）：roadmap §1 改 7 号方案——v1.1 background-image（+坑79）/ v1.2 border-radius ✅，v1.3 ColorFilter+九宫格 ✅，v1.4 列表+soft clip / v1.5 Controller-Gear-Transition / v1.6 富文本 / v1.7 TextInput-IME。补丁不占号（坑79 归 v1.1）。旧批序保留作对照列。

**v1.3+ 动态树重构完成**（`75d2646..6d0cd02`，spec `2026-07-01-v1.3-dynamic-tree-design.md`、plan `2026-07-01-v1.3-dynamic-tree.md`）：插在 v1.4 列表前的地基重构（非功能号）。v1 static-tree（pkg 加载即冻结）撞墙——v1.4 列表/v1.5+ 全需运行时改树。SDD 8 task + final review（2 Minor fix）全 Approved。**代际 NodeId**（u32，20bit idx+12bit gen+INVALID）+ **slotmap 存储**（DefaultKey 桥接，new_key_type! 不可行因 KeyData 64bit，坑 96）+ **持久附属 HashMap**（anim/scroll，SecondaryMap 不可行因 Key unsafe+位宽，坑 96）+ **动态 API**（create_root/create_node/append_child/insert_before/remove_child/set_text/set_src/set_style，CSS 串复用 mapping::apply_decl）+ **remove_node 联动清**（anim/scroll/tween/focused_node，坑 99）+ **world_transforms 时机前移**（T4，删首帧 guard + hit_test bounds guard）+ **base/style 分离**（代码已落地）。spec 写 plan 期修正：A4→A4'（SecondaryMap→HashMap）。踩坑：坑 96（slotmap KeyData 64bit）、坑 97（AnimTable 读写不对称）、坑 98（并行数组 len 分配越界→capacity+1）、坑 99（remove_node 漏清 focused_node）。core 468 测全绿。**showcase §3.10 四用例**（建/删子树+set_style toggle+批量20+动态加载 bin 切 mail/showcase pkg，LoomStage.LoadPackageFile + EventHandler.Clear）。**待家里机 PlayMode 验收**：§3.10 四用例 + 旧 §1-§9 零回归。机制详见 §2.29，坑 96-99。**已知潜伏债**（非阻塞）：flag-pack sentinel（bit 29/30 与 20bit idx 理论重叠，idx≥262144 才爆）+ version 12bit 截断（同槽复用 ~2048 次 wrap，安全返 None）。C# 改动本机未编译，家里机首编若有语法错待修。
