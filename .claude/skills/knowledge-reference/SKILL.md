...
| 文档 | 性质 | 何时读 |
|---|---|---|
| `docs/design/main-design.md` | **设计权威契约**（当前实现真相源） | 查设计意图/契约（§3 围栏 §4 parse §5 Node §6 taffy §7 render §8 text） |
...
**根因**：LoomGUI 契约 div 永远 flex column（main-design §3.1），Chromium 默认 div=block/content-box/body 8px margin → 不挂 polyfill 预览骗 AI（gap/flex-grow/align-items 全不生效）。polyfill 靠手抄易漏。
