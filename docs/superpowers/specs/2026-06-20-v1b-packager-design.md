...
**StringTable 内容**：text content、image src（可重复，去重有效）。`font_family` 随 StyleRecord 的 bincode blob 走（§5，不单列）；类名/tag 不进——parse 期产物，`build_scene` 后 Node 不再持。
...
```
Header (28B 定长):
  magic         u32 = 0x474B504C   // 磁盘字节(LE) = 4C 50 4B 47 = ASCII "LPKG"
                                   // （不与 frame blob 的 "LOOM" 撞 magic）
  formatVersion u32 = 1
...
StringTable:
  repeat stringCount:
    len   u16
    bytes [u8; len]  UTF-8          // 索引 = 顺序号（ReadS 风格）；只放 text content + image src

NodeBlock (nodeCount 条，DFS 先序；parentIndex 指数组下标):
  per node:
    parentIndex i32                 // -1=根
    kind        u8                  // 0=Container 1=Button 2=Image 3=Text
    styleLen    u32                 // 紧跟的 style blob 字节数
    style       [u8; styleLen]      // bincode(ResolvedStyle)，见 StyleRecord
    textIdx     u16                 // Text 的 content；非 Text = 0xFFFF(null)
    srcIdx      u16                 // Image 的 src；非 Image = 0xFFFF(null)
```

### StyleRecord
`ResolvedStyle`（`style/resolved.rs`）的 **serde + bincode** 编码（每个 node 的 `style` 字段 = `u32 len` 前缀 + bincode 字节流）：
- `ResolvedStyle` + `TextAlign` 加 `#[derive(Serialize, Deserialize)]`；`taffy_style: taffy::style::Style` 开 taffy `serde` feature（0.5.2 全字段类型都 `cfg_attr(feature="serde", derive(...))`）→ 整个 ResolvedStyle（含 taffy_style 全字段 + 14 视觉字段 + font_family）一个 bincode blob 搞定。
- **exhaustive 由构造保证**：serde 派生覆盖 ResolvedStyle 全字段；加字段 → 派生自动覆盖，encode/decode 永远同步，编译期强制（R3 近零）。
- `font_family` 随 blob 走（**不**进 stringTable）；stringTable 只去重 text content + image src。
- 字节序：bincode 默认 LE；version 钉 bincode 1.x（taffy 升级时 bump package formatVersion）。
...
- `magic != PKG_MAGIC(0x474B504C)` → `Err(PkgError::BadMagic)`。
