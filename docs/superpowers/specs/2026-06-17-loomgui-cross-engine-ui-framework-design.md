...
> 变更：v1→v2 渲染契约从"扁平 draw-call 流"改为"渲染树+后端镜像原生 GameObject"（支持 Unity 特效/世界空间 UI），改名 LoomGUI；v2.1 去掉 NativeHandle、scraper feature-gate、运行时只走二进制；v2.2 文本 mesh 归后端、RenderNode 公共头+enum payload、slot 复用、SOA+arena FFI、滚动/输入消费/参考分辨率；v2.3 TextLayout SOA 三表、打包器提前到 v1、图集优先级拉高、文本砍 BiDi、v1 平台收窄、围栏 v1 冻结清单。
...
### 4.7 v1 围栏冻结清单（动工前签字，防范围飘）
**元素**：`div`(Container) / `span`+裸文本(Text) / `img`(Image) / `button`(Button)。
砍：`l-rich`/`input`/`l-graph`/`l-loader`/`l-movie`/`l-list`/`l-slider`/`l-combobox`/`l-tree`/`l-native`（v1.x）。v1 可滚动列表用 `div` + ScrollPane 手搓几个 item，**不做 `<l-list>` 虚拟化**（§13.2 v1.x）。
...
砍：`position:absolute`、`align-content`、`row-gap/column-gap`（用 gap 统一）——v1.x。
...
**目标市场**：亚洲/国内首发（决定文本可砍 BiDi，§9.1）。**平台**：v1 仅 Win/Mac 桌面 + Mono backend（IL2CPP/移动端 v1.x，§18 R13）。
...
> 运行时**完全不走 HTML 解析**——打包器（`loomgui_pkg`，§12.1）把 DSL 编译成二进制包（§12），运行时从二进制建场景图（热重载也是重编译二进制再加载）。解析层（scraper/cssparser/selectors）在打包器/编辑器/工具链，feature-gate 不进运行时（§16.1）。打包器是 HTML→二进制的唯一编译器，复用本层的 parse/style 代码。
...
### 9.1 测量与渲染的分离（一致性根基）
- **Rust 核心拥有测量 + 断行**（确定性，跨引擎一致）：ttf-parser 取真实度量（`hhea`/`os2` ascent/descent/line-gap，**不照搬 fgui 的 `fontSize*1.25` 估算**）+ unicode-linebreak（换行机会，CJK 逐字）。
- **v1 范围（亚洲/国内首发）**：Latin 逐字 advance + CJK 逐字，简单断行。**砍 unicode-bidi（RTL）+ rustybuzz 复杂 shaping（连字/合字）**，推 v1.x。代价：v1 不支持阿语/希伯来/复杂连字/Emoji ZWJ。前提：首发市场亚洲/国内（已定）。若后续含中东/全球，BiDi/复杂 shaping 必须补。
...
```
**跨 FFI 时 SOA 三表化**（§14.3）：TextLayout 不进通用 arena，单独三张扁平表——`glyphs_soa[]`（扁平所有字形）、`runs_soa[]`（每 run = glyph 起止 + font + format）、`lines_soa[]`（每行 = run 起止 + y/height/baseline/width）。Text payload 只带六个 u32 指向三表切片。**cluster 现在就带**（v1 只读文本用不上选区/光标，但 cluster 是 u32 极便宜，v1.x 的 IME/光标/选区要用，避免改表结构）。v1 不做富文本，`inline_objects` 那层不进表；v1.x 加第 4 张 `objects_soa[]`。
...
- **二进制包由打包器产出**（`loomgui_pkg`，v1 就有，非编辑器）：HTML+CSS+资源清单 → `.pkg.bin` + 图集。打包器复用核心的 parse/style 层（§5），是 HTML→二进制的唯一编译器。这是 v1 能开工商的前提（否则运行时无包可加载，§18 R9）。
...
> **图集是 v1 刚需，非 v1.x 锦上添花**：同图集的图才能批合（§8.4），散图每张一个 TextureView = 每张一个 draw call，UI 一复杂就崩性能。v1 起步允许散图（先打通管线），但**图集打包紧随、优先级拉到 v1 早期**，打包器 `loomgui_pkg` 内置（散图→大图 + AtlasSprite 表：rect/offset/originalSize/rotated）。算法 v1 用简单矩形打包（shelf/guillotine），够用即可，后期可换最优算法。rotated 一并做（白送的纹理利用率，UV 修正 §8.1/§8.3 已预留）。
...
1. RenderNode 公共头 SOA（定长字段并行存储）：
   ...
   color_tints[], transforms[], blends[], clips[], sort_keys[], payload_kinds[],
   payload_arena_idx[], payload_offsets[], payload_lens[]
   —— 定长。payload_kinds 区分 Mesh/Text/Stencil*/Unchanged 等；
      (arena_idx, offset, len) 三元组定位该节点 payload 在哪个 arena 的哪段。
   不 dirty 的节点 payload_kinds = Unchanged，(offset,len) 为空，C# 不重传。
2. 多个按类型分区的 per-frame arena（变长 payload 内容，每种类型一个 arena）：
   ...
   stencil_arena  : 定长 stencil state（ref/compare/op/read_mask/write_mask/color_mask）
   —— 每种 arena 一种结构，C# 按 payload_kind 选解析器，零嵌套、零猜布局。
```

**变长 payload 的 SOA 化（核心规则）**：任何变长 payload 都拍平成扁平 SOA，**禁止嵌套结构跨 FFI**。
- **TextLayout**：三表 `glyphs_soa[] / runs_soa[] / lines_soa[]`（§9.2），Text payload 的 `(offset,len)` 指向三表各自切片。C# 读三张定长表，O(1) 跳转，零嵌套解析。
- **mesh**：已是扁平数组（verts/uvs/colors/indices + count），天然 SOA。
- **stencil**：定长 state，无需 SOA。
- **byte 布局定死**：每变体在 arena 内的字段顺序与对齐固定（如 mesh = `count:u32 + verts[n*2 f32] + uvs[n*2 f32] + colors[n u32] + indices[m u16]`），写进契约附录。
- **C# 读取策略**：用 `Span<byte>` + `BinaryPrimitives`/显式偏移读，**禁用 `Marshal.PtrToStructure`**（IL2CPP 下 struct 对齐/嵌套坑大）。每种 payload_kind 一个手写 reader。

**内存模型**：
- 公共头 + 各 arena 都是 **Rust 侧 per-frame**。C# 在 `tick` 返回前**拷贝完**到自己的预分配托管 buffer（拷贝而非 pin——避免 Rust 下帧 reset 时 C# 还在读的生命周期纠缠）。
- Rust 下帧开头 **reset 所有 arena**（不释放、复用）→ 零分配。C# 侧 buffer 也预分配复用 → 零 GC 压力（Unity 非分代 Boehm GC，每帧 `new` 会卡帧，必须避免）。
- **"沿用上帧 mesh"**：不 dirty 的节点 `payload_kinds = Unchanged`，不进任何 arena；后端见 Unchanged → 不动该 NodeId 的 MeshFilter（上帧已上传的 mesh 留在 Unity Mesh 里）。这把"沿用"语义落到 payload 变体，消除契约缺口。
- **拷贝 vs pin**：选拷贝。代价是每帧一次 memcpy；公共头 SOA 是几十 KB，memcpy 可忽略；变长 arena 按实际 dirty 量。pin 的生命周期纠缠不值。
- **升级路径**：v1 用"全集 SOA 公共头 + 按需 arena + 后端 diff"。成本是 O(可见节点数) memcpy，实测大概率够；不够再升级"变更列表"（核心已用 dirty 跟踪，能产出）——v1 不做，留路径。
...
├── loomgui_pkg/            # 打包器 CLI（v1）：HTML+CSS+资源→.pkg.bin+图集，复用 core 的 parse feature
...
| `rustybuzz` | 文本 shaping | 中（纯 Rust） | v1.x（亚洲首发 v1 不需要复杂 shaping） | v1 先 Latin/CJK 逐字 advance；rustybuzz 连字/合字推 v1.x（含中东/全球市场时必须） |
| `ttf-parser` | 字体度量 | 轻 | **是**（v1 必需，一致性根基） | 保留 |
| `unicode-linebreak` | 换行（CJK 逐字） | 轻 | **是** | 保留 |
| `unicode-bidi` | BiDi（RTL） | 轻 | v1.x | 亚洲/国内首发 v1 不需要；RTL 市场时补 |
...
### v1 最小可用（Unity 上能跑）
- **打包器 `loomgui_pkg`**：HTML+CSS+资源 → `.pkg.bin` + 图集（v1 早期，§12.1/§12.3）。开工商前提。
...
- **参考分辨率缩放**：设计稿等比缩放（§7.4）。
- FFI：csbindgen 通路 + SOA+多 arena 渲染树同步（§14.3）。
- 资源：二进制包加载 + 图集 TextureView（散图起步、图集紧随）+ refcount。
- 围栏：§4 子集的**冻结清单**（动工前签字，见下）。

### v1.x 增强
- 富文本 + 内联对象；九宫格/平铺/填充；软裁剪/模板遮罩/paintingMode；动画(GTween)+Transition+Controller+Gear；列表虚拟化（建在 ScrollPane 上）；滚动分页/吸附/下拉刷新；动态节点 API 完整化；自定义控件扩展；**IME 完整链路 + 软键盘（移动端）**；**字体 fallback 链**；**NativeHost（Unity 特效接入）**；**rustybuzz 复杂 shaping + unicode-bidi（RTL）**；**IL2CPP + 移动端（iOS/Android）**。
...
| R8 | **GameObject 镜像同步正确性** | diff 渲染树增删复用 GameObject（含 slot_id 复用、stencil eraser 独立 GO、Unchanged 沿用），易出 reuse/泄漏 bug。 | node_id 池 + slot_id 复用规则 + 脏粒度 + 单测；参考 fgui DisplayObject 生命周期。 |
| R9 | **DSL 产出/打包器** | v1 不做可视化编辑器；HTML 手写 + 打包器产出二进制。团队是否有美术能写 HTML/CSS？ | **打包器 `loomgui_pkg` 提前到 v1**（§12.1）解鸡生蛋；最小只读 WASM 预览评估提前；程序员先手写 HTML 起步。 |
| R10 | **团队 Rust 经验** | 核心全 Rust（taffy/rustybuzz/csbindgen），若团队以 C#/Unity 为主，学习曲线陡。 | 先做 v0 纯 Rust 骨架（HTML→打包→taffy→render_nodes JSON 快照）验证 Rust 能力，1-2 周 go/no-go。 |
| R11 | **Unity Domain Reload / 热重载** | 编辑器进出 Play mode 丢 native 状态；改 Rust 重编译要重启 Unity；static 回调失效。 | StageEngine 加 `[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` 重置（fgui 同款）；接受"改 Rust 必重启"。 |
| R12 | **Rust↔Unity 联调链路** | Rust 在 native、C# 在 Unity，断点/日志/profiling 跨边界难。 | 核心先纯 Rust 单测 + 快照测试（DSL→render_nodes JSON）覆盖 90%；Rust `RUST_LOG` + Unity 双日志；Unity 只验镜像同步。 |
| R13 | **csbindgen 生成代码 + 多平台构建** | 生成代码随 Rust API 变频繁变；交叉编译（iOS/Android）工具链复杂。 | v1 仅 Win/Mac 桌面 + Mono backend（IL2CPP/移动端 v1.x）；生成代码单独 asmdef；交叉编译脚本 v1.x。 |
| R14 | **FFI 双 buffer 拷贝性能滞后** | §14.3 选拷贝未实测；v1 末才发现卡帧则返工大。 | v1 中段做 500 节点 stress 测试早暴露。 |
