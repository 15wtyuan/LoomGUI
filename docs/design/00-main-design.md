...
### 1.1 要解决的问题
传统游戏 UI（Unity UGUI / Godot Control）布局能力弱（锚点/绝对定位为主），跨引擎不通用，各引擎渲染表现不一致。FairyGUI 证明了"独立编辑器 + 跨引擎运行时 + 自绘"路线可行，但它的布局仍是锚点式、DSL 是私有二进制。LoomGUI 做它的精神继承者：HTML+CSS 子集作 DSL、flexbox 替代锚点、Rust 核心覆盖多引擎。
...
│   解析层(scraper+cssparser+selectors) → 样式层(cascade)         │
...
### 4.1 围栏总原则与忽略策略
- **容器默认 `display: flex`**（`flex-direction: row`）。有意背离浏览器默认 block——布局模型就是 flexbox。纵向堆叠写 `flex-direction: column`。
- **文本是节点不是流**：纯文本包成叶子文本节点，不做浏览器行内混排。
- **自定义元素 kebab-case**：`<l-list>`/`<l-loader>` 等用 `l-` 前缀避免与 HTML 冲突。
- **忽略策略分级**（关键）：
  - **装饰属性**（color/border-radius/filter/background-position…）：静默忽略 + 告警，布局语义不变。
  - **布局语义属性**（display 非 flex/none、position:absolute/sticky/fixed、float、grid）：**编译期报错**（打包器拒收），不允许静默 fallback——必然导致布局错误，早报比晚报好。
...
- 元素内"文本+元素+文本"混排（如 `<div>hello <img> world</div>`）：v1 不支持行内混排，文本 siblings 降级为独立 Text 子节点（flex 排成块），告警。
- `<b>/<i>/<u>` 在普通 div/span 内：忽略标签但保留文本内容，告警；内联格式化用 `<l-rich>`。
...
### 4.4 支持的 CSS 属性
**布局类（→ taffy）**：`display:flex/none`、`flex-direction`、`flex-wrap`、`gap`、`justify-content`、`align-items`、`align-self`、`align-content`、`flex`(grow/shrink/basis)、`width/height/min/max`(px/%/auto)、`padding`、`margin`、`border` 宽度、`position:relative/absolute`、`top/right/bottom/left`、`aspect-ratio`、`order`。
...
**九宫格**：`border-image-slice` / `-l-slice`（Image 九宫格切分）。

### 4.5 状态与控制器
浏览器伪类 `:hover/:active/:focus/:disabled` 映射内置状态。自定义状态（fgui Controller 多页）用 **data 属性 + 自定义伪类**：
```html
...
```
`:l-page(n)` 是运行时状态驱动的伪类（Controller.selected_index），通过 selectors 的 `NonTSPseudoClass` 扩展 + Element 适配器匹配（见 §5.3）。带过渡用 `-l-transition: 0.3s ease`（映射 GTween）。
...
- **CSS 声明解析**：`cssparser`（Servo）解析 `{ prop: value; }` 声明块。selectors 只做选择器匹配，不解析声明，故 cssparser 必装。
- **选择器匹配**：`selectors` crate，支持标签/类/id/后代/子代/`:nth-child`/属性选择/自定义伪类。

### 5.2 Cascade 子集（三要素，必须完整）
1. **来源优先级**：`inline > id > class > tag`。
2. **同优先级按出现顺序**：后者覆盖前者。
3. **属性级合并**：多规则命中同一元素，逐 longhand 取最高优先级值。`flex` 简写按 MDN 展开（`flex:1`→grow=1,shrink=1,basis=0%），与单独 `flex-grow` 同优先级时按出现顺序。
4. **继承**（关键，不实现则 DSL 不可用）：白名单 inherited properties 默认从父继承——`color`、`font-size`、`font-family`、`font-weight`、`font-style`、`line-height`、`letter-spacing`、`white-space`、`text-align`。其余 non-inherited。

**打包期展开继承**：打包器在编译期就把继承展开成每节点的 `base_style`（构建期树是静态的可算）→ 运行时零继承开销，二进制包里每节点带基础样式。这才是打包器的核心价值。

### 5.3 运行时伪类匹配与样式 dirty 机制
`:hover/:active/:disabled/:l-page(n)` 都是**运行时状态驱动的伪类**，selectors 需运行时对每个元素求值。机制：
- **Node→selectors::Element 适配器**：把 Node 树暴露成 `selectors::Element`，匹配伪类时回调查状态（hover=输入命中、active/disabled=节点属性、`:l-page(n)`=Controller.selected_index）。
- **样式 dirty 档**：DirtyFlags 增加 `style` 档（见 §6.3）。Controller/输入状态变化 → 标子树 style dirty → 重新跑 cascade（selectors 匹配 + 合并 + 继承展开）→ 置 layout dirty。
- **匹配结果缓存**：按 `(selector, 状态指纹)` 缓存，状态指纹变才重匹配。
- v1 至少实现 `:hover/:active/:disabled`（按钮交互必需）；`:l-page(n)`+Controller/Gear 后续。
...
2. **动态规则表**：带伪类/状态的选择器规则集（`:hover`/`:l-page`/`:nth-child`），声明以"规则→属性映射"存，运行时 cascade 叠到 base_style 上。
...
struct Node {
    ...
    // dirty（分层）
    dirty: DirtyFlags,          // style/mesh/layout/batching/outline/transform
    // 事件
    listeners: HashMap<EventType, EventBridge>,
    // 状态/动画
    gears: [Option<Gear>; 10],
    gear_locked: Cell<u8>,      // 深度计数，防 reentrancy（非单 bool）
    // 子节点（仅 Container）
...
- **图片**：原始像素尺寸或声明尺寸。
- **RichText 内联对象**：先 query 每个 img/input 的 (w,h) 再参与断行（在测量回调内部，不经 taffy）。
...
- 批合局部（每 BatchingRoot 独立）；裁剪/遮罩/paintingMode 天然是 root 边界。
...
- **FairyBatching 重排不得跨越 Erase**：批合 root = 裁剪/遮罩边界，Erase 是子树末尾硬边界。
- **九宫格**：核心九宫格 MeshFactory 生成 16 顶点 mesh，作为普通 Mesh payload，不进材质。
- **slot_id**：虚拟化复用键。后端 diff 按 slot_id 复用渲染对象（旧 item 滚出、新 item 滚入同 slot → 复用同对象），非按 NodeId 销毁重建（§13.2）。
- **Unchanged**：不 dirty 的节点用此变体，不进 arena；后端见 Unchanged → 不动该 NodeId 的渲染对象。**注意**：Unchanged 是独立变体（非 dirty_bits 位），enum 只留真实 payload 类型，避免语义混淆。
- **NodeTransform**：本地变换 + pivot 偏移 + `Option<VertexMatrix>`（透视/斜切，世界空间 UI）。

后端每帧：diff `render_nodes` 与镜像池（按 node_id 增删、按 slot_id 复用）→ 同步对应 payload（Mesh 上传 mesh、Text 据 layout 生成 mesh、Mask 建/更新遮罩对象）→ 设 transform/排序/遮罩/blend。
...
- "可能演进"的字段（VertexMatrix/动画权重/tessellation）一开始就留**可选扩展列**（offset 指针，存在才读），不塞定长 SOA。
...
```
**跨 FFI 时 SOA 三表化**（§14.3）：TextLayout 不进通用 arena，单独三张扁平表——`glyphs_soa[]`（扁平所有字形）、`runs_soa[]`（每 run=glyph 起止+font+format）、`lines_soa[]`（每行=run 起止+y/height/baseline/width）。Text payload 带六个 u32 指向三表切片。**cluster 现在就带**（v1 只读用不上，但 v1.x IME/光标/选区要用，避免改表）。富文本内联对象加第 4 张 `objects_soa[]`。
...
- **双向同步 reentrancy 安全**：`gear_locked: Cell<u8>`（**深度计数，非单 bool**）区分哪个 Gear 在写。GTween on_update 回调里 set 属性→update_gear：若当前是 Gear.Apply 触发的写（locked>0）则跳过回写，避免污染 Gear 存储。
...
- **惯性回弹物理**：**不走 GTween**（content 异步变化时 GTween 的固定 end 会跳变）。ScrollPane 自维护可变 target 的 tween（`_tween_start/_tween_change/_tween_time`，越界截断重启回弹），content size 变化时 `change_content_size_on_scrolling` 按状态补偿 `_tween_start`/`_container`，不突变。这是 §11 单时钟原则的**明确例外**（理由：动态 content 需可变 target）。
...
- RenderNode 带 `slot_id`（§8.7）。后端 diff **按 slot_id 复用渲染对象**（同 slot 只更新内容），不销毁不重建 → 零 GC 抖动。
- **两身份正交**：`NodeId`=逻辑身份（事件/命中/核心对象），`slot_id`=渲染复用身份（后端渲染对象池 key）。普通节点 slot_id=None。
- **slot 换内容强制 dirty**：slot 命中 + NodeId 变 = 必重传 mesh/TextLayout（覆盖"沿用上帧"）。
...
```
**变长 payload 全 SOA 化**：任何变长 payload 拍平成扁平 SOA，**禁止嵌套结构跨 FFI**。每变体 byte 布局定死（写进契约附录）。**C# 用 `Span<byte>` + `BinaryPrimitives` 读，禁用 `Marshal.PtrToStructure`**（IL2CPP struct 对齐坑）。

**内存模型**：公共头 + 各 arena 都是 Rust 侧 per-frame。C# 在 `tick` 返回前拷完到预分配托管 buffer（拷贝而非 pin）。Rust 下帧开头 reset arena（复用零分配）；C# buffer 预分配复用（零 GC）。**"沿用上帧"**：不 dirty 节点 payload=Unchanged，不进 arena，后端不动其渲染对象。
...
4. **NativeHost**：放用户 GameObject（粒子/3D/自定义），按布局 transform/clip 放置，不画自有 mesh。
...
- **每帧脏增量同步**（非全量重刷）：后端 diff 渲染树——新增 NodeId 建对象、消失销毁、状态变更新、没变 no-op。静态 UI 每帧同步≈0。
- 真正每帧开销是引擎自身遍历渲染对象做剔除/批合/提交——靠 DrawState 复用 + FairyBatching 缓解。纯 2D 重 UI 不够 → 升级 SRP 混合。
- **句柄**：Rust 不持任何 Unity 句柄。所有 GameObject（自绘+NativeHost）后端拥有，后端维护 `NodeId→GameObject`。NativeHost 用户 GameObject 经后端 API `BindNativeHost(nodeId, go)` 注册。
- **回收**：节点 Dispose → 下帧不在渲染树 → 后端销毁镜像（或核心发"已移除列表"立即清理）。虚拟列表 item 按 slot_id 复用渲染对象（不销毁重建）。NativeHost Dispose = detach（不销毁，用户拥有）。
...
引擎 update:
  ...
  2. stage.tick(dt):
     a. Timers.update(dt)
     b. TweenManager.update(dt)        ← 唯一动画时钟；tweener 回调写节点属性（置 transform_dirty/layout_dirty）
     c. style dirty → 重 cascade（selectors 重匹配 + 合并 + 继承展开）→ 置 layout dirty
...
│   ├── parse/              # HTML(scraper) + CSS(cssparser) + selectors —— feature 'parse'，运行时不带
│   ├── style/              # cascade（继承/合并/顺序）+ CSS值→taffy映射 + selectors::Element 适配
