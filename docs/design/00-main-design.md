...
4. **围栏优先 + 标准标签**：HTML/CSS 只支持明确子集（§4）；围栏只暴露**标准 HTML 标签**（`div`/`span`/`img`/`button`），不自创 `l-` 前缀标签——AI 训练数据里没有的自创标签损害可预测性（§4.1）。
...
> LoomGUI 只支持 HTML/CSS 的一个明确子集，称"围栏"。**围栏属性权威清单 = `docs/design/fence.md`**（其真相源是可执行测试 `loomgui_core/tests/fence_contract.rs`，不一致时测试赢）。**本节只写设计哲学与原则，不重复属性表**——重复维护即漂移根源（§4 反复过时的根因）。改围栏属性改 fence.md + 测试，不改本节。

### 4.1 设计哲学：标准标签 + AI 可预测性

**首要判据**（§1.1）：AI 读 HTML 能否正确预测渲染结果。所有围栏决策的第一判据。

**标准 HTML 标签优先**：围栏只暴露 AI 训练数据海量、浏览器原生渲染的**标准标签**——`div`/`span`/`img`/`button`。**不自创 `l-` 前缀标签**：早期的 `l-container`/`l-rich`/`l-list` 等已摒弃——`l-container` 与 `div` 同映射是冗余假元素；自创标签 AI 训练数据里没有，见了反而困惑"该用 div 还是 l-container"，且 Chromium 预览不认会塌成一坨。
- `l-` 前缀原则**保留但当前围栏不暴露任何 l- 标签**：留给未来确有独特语义、无标准等价物的"真·自定义元素"。
- **虚拟列表/富文本不暴露 `<l-list>`/`<l-rich>`**：runtime 行为（slot 复用/行内混排）由代码层在 ScrollPane/文本测量上实现（§13.2）；设计师用 `div`+`gap` 画 item 模板、纯文本占位即可。AI 不知道这些标签就不会写。
...
围栏外处理分两类（实证见 fence.md §0 方法论 + fence_contract.rs，非源码 grep 推测）：

- **围栏外标签**（如 `<video>`/`<input>`/`<b>`/`<section>`）+ **行内混排**（元素内"文本+元素+文本"）：**编译期报错**（parse 期失败、打包器拒收，不降级）。这是"写什么得到什么"的口径。
- **围栏外 CSS 属性**（如 `position:absolute`/`float`/`clip-path`/`cursor`/`font-style`）：**静默忽略**（`apply_decl` 返 `false`，字段不变，布局语义不变）。
  ...
  - 这些"静默忽略"行为本身**被测试锁定**，不可靠推测——fence.md §0 的 position:relative 教训："搜索代码无 match"≠"不支持"，可能是底层默认。

> 围栏外标签/CSS/选择器的完整清单见 fence.md §1（元素）/§2.4（围栏外 CSS）/§3.3（围栏外选择器）。

### 4.3 支持的元素 / CSS / 选择器

**权威清单见 fence.md**，本节只列大类：
- **元素**（fence.md §1）：`div`(Container) / `span`+裸文本(Text) / `img`(Image) / `button`(Button)。围栏外标签报错。
- **CSS 属性**（fence.md §2）：布局（flex 全家 + 尺寸/padding/margin/border-width/order/aspect-ratio）、视觉（background-/border-/opacity/overflow(+x/y)/color/font-*/text-align/line-height/letter-spacing/white-space/transform）、交互（pointer-events）。**v1.x 已实现**：`filter`(v1.3 颜色矩阵)、`border-image-slice`(v1.3 九宫格)、`border-radius`(v1.2 圆角)。值约束（如 background-size 仅 cover/contain/100% 拒两值）见 fence.md。
- **选择器**（fence.md §3）：标签/类/id/后代/子代/分组 + 伪类 `:hover/:active/:disabled/:focus`（:focus v1d.2 已实现）。
...
- **防漂移门**：`cargo test -p loomgui_core fence_contract`——本机 build .dll 前、改 `apply_decl`/`FENCE_TAGS`/selector 后必跑。
- **核实方法论**：改围栏前查依赖默认值（taffy `Style::DEFAULT`）+ 补测试，不靠 grep 推断。

### 4.5 状态与控制器 *(v1.x Controller/Gear)*
浏览器伪类 `:hover/:active/:focus/:disabled` 映射内置运行时状态（v1 已全部实现，§5.3 伪类 rematch）。自定义状态（fgui Controller 多页）用 **`data-page` 属性 + 标准属性选择器**（AI 烂熟 `data-*`）：
```html
...
### 4.6 预览可信（v other 编辑器）
open-design Chromium iframe 预览 ≠ taffy（字体度量/flex/margin 折叠/position:absolute 脱离流等分歧）。AI 须"信围栏规则，别信预览不可信项"——可信/不可信清单见 fence.md §6。v2 用 WASM 跑核心做零偏差预览替换。
...
- `ProgramId`：Image(0)/Text(1)（+ v1.x 已实现：BG_COMPOSITE(2)=Container+background-image 合成、ColorFilter(3)=颜色矩阵叠加；BMFont/自定义后期）。
...
> **v1 文本简化假设与已知代价**（CJK+emoji→tofu、组合符号→错位、RTL 不支持）、**v1.x 跨引擎归一化契约升级**（Godot 接入时定 advance/metric 权威、关 hinting）见 `docs/roadmap/roadmap.md`（机制草稿节）。
...
> **实现状态**：v1c.3-v1d.5 已落地——多触摸(5 槽)、CaptureTouch、click 全对齐 fgui（双击/cancel/阈值）、drag、longpress、键盘 keydown/up、焦点+Tab+`:focus`、transform 命中渲染、GTween、ScrollPane+滚轮+手势仲裁。机制细节见 knowledge-reference §2.15-2.21 + 各 spec。
...
- v1 能力：滚动类型、惯性+回弹、滚动条、鼠标滚轮。分页/吸附/下拉刷新、虚拟化列表 v1.x（不暴露 `<l-list>` 标签，§13.2）。
...
### 13.2 数据驱动的列表虚拟化 *(v1.x)*
建在 ScrollPane 上（**不暴露 `<l-list>` 标签**——围栏只有 div/span/img/button，§4.1；宿主用 `create_node` 建 item 模板，核心做 slot 复用，AI 不知道 l-list 就不会写）。核心维护固定数量可视槽（item index → slot），后端按 slot 复用渲染对象（不销毁重建，零 GC）。两身份正交：NodeId=逻辑身份（事件/命中），slot=渲染复用身份。**slot 复用的核心不变量**（slot 换内容时必发真实 payload 非 Unchanged，防花屏）与 reuse_key 机制见 roadmap（机制草稿）。
...
1. RenderNode 公共头 SOA（定长字段并行存储，**当前 18 列 / blob v4**）：
   ...
   `world_matrix(m_a,m_b,m_c,m_d,m_tx,m_ty — 6 列累计世界矩阵，v1d.3 替原 local_x/local_y),`
...
引擎 update:
  ...
  2. stage.tick(dt) — 内部固定顺序（v1d.5 定稿）：
     ...
     i. build_render_nodes              ← DFS：mesh/text dirty 重生成几何 + **dirty hash 比 → Unchanged emit**（v1e，静态帧≈0 upload）+ FairyBatching 重排 + 合成 scrollbar + 分配 sort_key
