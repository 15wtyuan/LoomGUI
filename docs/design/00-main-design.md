...
### 4.1 围栏总原则与忽略策略
- **容器默认 = 垂直堆叠**（browser-compatible）：`<div>` 不写 `display` 时子节点垂直堆叠（实现上 = flex-column，引擎仍是 flexbox，仅默认方向 column）。要水平排列显式写 `display: flex`（= row，与 CSS 一致）。**这么定是为了 AI 可预测性**——AI 对 `<div>` 的先验是浏览器 block=垂直堆叠，默认对齐此先验，AI 读 HTML 即可预测布局（§1.1 核心动机）。
- **不做行内流**：纯文本包成叶子文本节点，不实现浏览器行内混排（多 span 水平流、图文环绕）。多段文本用单个 text 元素或 `<l-rich>`。这是与浏览器的已知 divergence，围栏文档须明确告知 AI（不要用多 span 拼行内）。
- **自定义元素 kebab-case**：`<l-list>`/`<l-loader>` 等用 `l-` 前缀避免与 HTML 冲突。
- **忽略策略分级**（关键）：
  - **装饰属性**（color/border-radius/filter/background-position…）：静默忽略 + 告警，布局语义不变。
  - **布局语义属性**（display 非 flex/none、position:absolute/sticky/fixed、float、grid、行内混排）：**编译期报错**（打包器/验证器拒收），不允许静默 fallback——必然导致布局错误，早报比晚报好。
...
- **行内混排不支持**（§4.1 已定走 A，不放给 AI 写）：元素内"文本+元素+文本"混排（如 `<div>hello <img> world</div>`）、多个 `<span>` 拼行内——验证器/打包器**编译期报错**，提示用单个 text 或 `<l-rich>`。不降级处理。
...
```
`:l-page(n)` 是运行时状态驱动的伪类（Controller.selected_index），由极简匹配器查状态匹配（见 §5.3）。带过渡用 `-l-transition: 0.3s ease`（映射 GTween）。
...
`:hover/:active/:disabled/:l-page(n)` 是**运行时状态驱动的伪类**，匹配时查节点当前状态。机制（朴素，无过度优化）：
- **极简匹配器查状态**：匹配伪类时直接查节点状态（hover=输入命中、active/disabled=节点属性、`:l-page(n)`=Controller.selected_index）。不用 selectors crate 的 Element 适配器回调（§5.1 已砍）。
