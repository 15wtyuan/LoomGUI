...
**视觉/渲染类**：`background-color`、`background-image`(url)、`background-size`(cover/contain/100%/tile)、`border`(color/width/solid)、`opacity`、`overflow`(visible/hidden/scroll/auto) + `overflow-x/overflow-y` longhand（v1d.5 ScrollPane）、`color`、`font-size`、`font-family`、`font-weight`、`font-style`、`text-align`、`line-height`、`letter-spacing`、`white-space`(nowrap/normal)。

**交互类**：`pointer-events`(auto/none)。

**色彩滤镜**：`filter`（标准 CSS 函数子集：grayscale/brightness/contrast/saturate/hue-rotate/invert/sepia → 4×5 颜色矩阵）。
...
>
> **v1c.4 实现状态（@ db33b24，待家里机验）**：click 全对齐 fgui 落地——`click_test`（Click 目标=down_targets[0] 按下叶非当前 hit，照 fgui ClickTest；per-axis 阈值 mouse10/touch50 **固定像素无缩放**，「缩放容忍」证伪删除；down_leaf 失效沿祖先兜底）+ 双击 clickCount 1→2→1（350ms+per-axis+同键；EventRecord `pad[0]`→`click_count:u8` offset5，20B 不变 → C# `EventContext.clickCount`/`isDoubleClick`）+ Move>50 取消（硬编码）+ **Canceled**（`PointerKind::Canceled=3`；**偏离 fgui quirk**=隐式 CancelClick：置 click_cancelled→不发 Click+reset，spec §0.6 用户确认；Up 仍发）+ CancelClick API（FFI `cancel_click` + C# `CancelTouch`）+ **stopImmediatePropagation**（纯 C# W3C，fgui 无；EventBridge GetInvocationList 逐回调 break）+ **Stationary hover 跟随**（process 头部无事件活跃槽 re-hit-test，fgui 局限 LoomGUI 改进，见下条修订）+ time_s 复用 tick(dt)（C# `unscaledDeltaTime`→`Stage::advance_time`，零新 FFI 参数）。**EventRecord 20B/PointerEvent 16B/PointerKind repr(u8)/frame blob/.pkg.bin 全零改**（click_count 复用 pad）。spec `docs/superpowers/specs/2026-06-24-v1c.4-click-design.md`。
> **v1d.1 实现状态（@ 0fac306，待家里机验）**：drag + longpress + safe-area 落地（开 v1d，检测全 core input.rs，机制镜像 fgui 已核实源码）。**drag**（opt-in `Node.draggable`，HTML `draggable="true"` 属性，pkg.bin v4 NodeBlock +flags byte formatVersion 3→4；core 状态机阈值 mouse2/touch10 per-axis **< click 容忍 10/50**，照 fgui `UIConfig.clickDragSensitivity/touchDragSensitivity`；drag_target=down_targets 内层 draggable（leaf 优先 `.iter()`）；DragStart 置 click_cancelled → drag 必取消 click；EVT 6/7/8；只发事件**不跟手**，跟手→v1d.3 transform）+ **longpress**（universal，tick 1.5s/50px 照 fgui LongPressGesture；process 头部空事件 tick 也跑；EVT 9；与 click 独立不自动取消）+ **safe-area**（纯 Unity 侧 `LoomStage.ComputeRootTransform` 读 `Screen.safeArea` uniform shrink-to-fit + `LoomInputCollector.ScreenToDesign` 逐项逆；**core 零改**；T8 Critical fix：offX 设计 span 居中 safe 区 + forward/inverse 同源互逆，否则触控↔渲染错位）。**EventRecord 仍 20B**（drag/longpress 复用 event_type 6-9）；version v1d.1；无新 FFI（csbindgen 不 regen）；.dll 重编（1694720B）。**M6 行为变化（spec §5.1 本意）**：设计 aspect≠屏 aspect 时 v1c per-axis stretch → v1d.1 uniform shrink-to-fit+letterbox 居中（v1c latent render/input 不一致修复）。spec `docs/superpowers/specs/2026-06-24-v1d.1-drag-longpress-safearea-design.md`、路线 `docs/roadmap/v1d-plan.md`。
...
- RollOver/Out：每帧命中后 diff 一次（非每 move）。**Stationary hover 跟随（v1c.4 落地，fgui 改进）**：v1c.3 照 fgui 局限「元素动后 hover 不跟随」（仅事件触发 diff）；v1c.4 `process` 头部对本帧无事件的活跃槽 re-hit-test `last_pos` + hover_diff——静止光标下元素动画/布局移入其下 → :hover/RollOver/Out 刷新（每帧每未占用活跃槽 +1 次 hit_test）。

> **v1d.2 实现状态（@ 8f2629e，待家里机验）**：键盘 keydown/up + 焦点 + Tab 导航 + `:focus` 伪类 + FocusIn/Out 落地（检测全 core input.rs，机制镜像 fgui 已核实源码）。**焦点**（单一全局 `Scene.focused_node: Option<NodeId>` + `Node.focused:bool`[:focus 源] + `Node.tabindex:Option<i32>`[HTML `tabindex` 属性 None/-1/0/N，`parse::<i32>().ok()` 非数字→None]；pkg.bin v4→**v5** NodeBlock flags 后 +`tabindex:i32`[None→`i32::MIN` 哨兵 round-trip] formatVersion MIN=MAX=5 拒 v4；`Scene::build` 6→7-tuple）+ **focus 通道单源** `pub(crate) fn focus_node(scene,new,out)`（设/清 focused_node+focused + 发 `FocusOut`@旧/`FocusIn`@新 同目标 no-op；**3 处共用**：Stage tick 最前消费 `pending_focus_request` / `process` Down arm **click-to-focus**[沿 down_targets 找最近 tabindex>=0 非 disabled，`.copied()` 释放 slot 借用] / `process_keys` Tab 导航）+ **request_focus 强制**（任意非 disabled 节点含 tabindex=None/-1，照 fgui RequestFocus 不查 focusable；不直写 last_events 记 `pending_focus_request` 下 tick 消费，避免 tick 覆盖丢事件）+ **Tab 链** `build_tab_chain`（正整数 tabindex 升序 stable 后接 tabindex=0 组 DFS 序，-1/None/disabled 排除）+ `next_focus`（链内 ±1 **wrap**，链外→首/尾）+ **Tab/Shift+Tab(`KEY_TAB=9`+`MOD_SHIFT`) 导航消费不发 keydown**（照 DOM Tab 默认动作=移焦；业务拦 Tab→v1.x preventDefault）+ **keydown/up**（新 `KeyEvent{key_code:u32,modifiers:u8,is_down:bool,pad:[u8;2]}` 8B 输入；有焦点才发，无焦点丢弃；**复用 EventRecord 流** `EVT_KEY_DOWN=12`/`UP=13`/`FOCUS_IN=14`/`FOCUS_OUT=15`[node_id=焦点节点、`touch_id`复用装 key_code、`pad[0]`=modifiers、x/y=0，**零新输出 ABI**]；modifiers bit0=shift/1=ctrl/2=alt 照 fgui 砍 cmd）+ **`:focus` 伪类**（`Compound.pseudo_focus` dynamic.rs 单一定义 selector.rs pub use 重导出 + `compound_matches_with_state` 门控 `node.focused` + `extract_dynamic_rules` 纳入 `:focus` 规则，rematch 每帧跑自动吃）。**tick 管线 +⓪消费 pending_focus_request +②process_keys**（⓪→solve→process→process_keys→last_events→rematch→render）。version v1d.2；**3 新常驻 FFI**（`set_key_input`/`request_focus`/`focused_node`）+ `KeyEvent` struct（csbindgen reimport regen）；.dll 重编（1694720→1709056B）。**EventRecord 仍 20B / PointerEvent 16B**（key 事件复用既有 event_type 空位 12-15 + 字段复用）。spec `docs/superpowers/specs/2026-06-24-v1d.2-keyboard-focus-design.md`、路线 `docs/roadmap/v1d-plan.md`。

> **v1d.3 实现状态（@ f431cb3，待家里机验）**：transform 命中渲染 + NativeHost-lite + 整树 dump 落地（矩阵全 core `transform.rs`/`scene/transform.rs`，机制镜像 fgui vertexMatrix 已核实源码）。**transform**（`Affine2=[a,b,c,d,tx,ty]` 列主序 + free fn + `Affine2Ext` trait；`LocalTransform{matrix:Affine2}` **存矩阵非 TRS 分解**——剪切复合 `scale(2,1) rotate(45deg)` 解析期 `mul` 累积不丢；`compute_world_transforms` 每 frame DFS `local=T(rel)∘T(pivot)∘matrix∘T(-pivot)`[**pivot=box center 固定**，rel=layout_rect.xy−父.xy，root rel=自身 xy]，`world=parent.world∘local`，借用安全算局部 Vec 末尾赋值；`Scene.world_transforms:Vec<Affine2>` + `RenderNode.world_matrix:Affine2` 替 NodeTransform）+ **渲染两路径**（§3.5b：identity/merge 走 TRS 零回归[顶点绝对+blob re-base 减 tx,ty→top-local+GO localPosition=tx,ty+现有 material]；**非纯平移** break merge 走 matrix shader[顶点 box 本地(0,0,w,h)+blob 不 re-base+GO transform=identity+`_ObjectMatrix` uniform+shader `OBJECT_MATRIX` variant 顶点 `mul(_ObjectMatrix,float4(pos.xy,0,1))`→world]；两路径判断同 `is_pure_translation(wm)` epsilon 1e-6 Rust↔C# 对齐；剪切任意仿射天然支持）+ **命中 world_to_local**（§3.4：`inverse(world).apply_point(point)`→本地 box `(0,0,w,h)`；仿射逆含剪切可逆；**clip 不随 transform 旋转**保守留 v1.x）+ **blob v3→v4**（transform 列 `local_x/local_y`(2)→world matrix `m_a..m_ty`(6)，18 列 header_len=84）+ **pkg v5→v6**（`ResolvedStyle`+transform bincode）+ **NativeHost-lite**（§3.7：后端纯 C#，div 占位 + `BindNativeHost(uint|string)`[复用 `find_node_by_id`]，TRS 分解跟随 world matrix + 显隐 + sort_key→sortingOrder；core 零改）+ **dump_scene**（FFI 整树 JSON，id/classes 转义）。**tick 管线 +compute_world_transforms**（⓪pending_focus→solve→**compute_world**→process[hit world_to_local]→process_keys→last_events→rematch→render）。version v1d.3；**1 新常驻 FFI**（`dump_scene`）+ StageHandle+`dump_blob:CString`；.dll 重编（1709056→1733120B）。**两机约束**：core+ffi cargo test 全绿（core 230+ffi 37+snapshot 3=270）；C# 本机未编译，家里机 PlayMode（旋转/剪切/缩放容器视觉+子跟随/剪切走 matrix shader/命中 world_to_local/identity 不回归/NativeHost cube 跟随+显隐/DumpScene 日志/500 节点 stress）。subagent-driven 11 task 全 Approved + final review Ready（opus，跨语言矩阵契约全链核实无 Critical；I1 Mesh.bounds 剔除+M1 共享 material MPB fix）。spec `docs/superpowers/specs/2026-06-25-v1d.3-transform-nativehost-design.md`、路线 `docs/roadmap/v1d-plan.md`。

> **v1d.4 实现状态（@ 24e2aec，待家里机验）**：GTween tween 引擎落地（全 core `tween.rs`/`stage.rs`，机制镜像 fgui GTween/EaseManager 已核实源码）。**TweenManager + replace-override**（`TweenProp`/`Ease` `#[repr(u8)]` 0..5/0..9 + `Ease::evaluate` 10 个照 fgui 直译 OVERSHOOT=1.70158 + `prop_value_size` 1/2/4 + `try_from(u32)` 边界校验；`TweenManager.update(dt,scene,&mut out)` delay 门控→ease→apply 写通道→complete；Stage 持 `tweens`）+ **anim override = Scene transient `anim:AnimTable`**（`NodeAnim{opacity/transform/bg_color/text_color}` 全 Option，同 `world_transforms` 不进 pkg；**4 读取点 replace-override** `unwrap_or(CSS)`——`compute_world_transforms` 读 `anim.transform` 覆盖 css_matrix **不 compose**、`build_render_nodes` 读 opacity/bg_color/text_color；`AnimTable::get` 经 `is_empty` 过滤→**全 None 零回归**；一节点一 transform tween[Translate/Scale/Rotation 共享通道 last-write-wins，混用嵌套 div]；持久停末值，`clear_anim(_prop)` 回 CSS）+ **时钟单 unscaled dt stash**（`advance_time` stash `pending_dt`，`tick_and_render`[**无参签名不动**]顶部 update，**须在 solve/compute_world_transforms 前**；load_inline/load_package 清 tweens 防悬空 node_id）+ **`EVT_TWEEN_COMPLETE=16` 复用 EventRecord 字段**（`click_count`=prop、`touch_id`=tag，**零结构改动 20B 不变**；C# `EventType.TweenComplete=16`+`DispatchPending` DirectDispatch[listener 读 ctx.clickCount=prop/ctx.touchId=tag]）。**tick 管线 +tweens.update@顶**（⓪pending_focus→**tweens.update**→solve→compute_world→process→process_keys→last_events→rematch→render）。version v1d.4；**4 新常驻 FFI**（`tween/kill_tween/clear_anim/clear_anim_prop`，csbindgen regen）+ `TweenProp/Ease::try_from`；**blob 保持 v4 不 bump**（anim 在 core fold 进既有字段，blob/MirrorPool 零改）；.dll 重编（1733120→1739776B）。**两机约束**：core+ffi cargo test 全绿（core 249+ffi 38+snapshot 3=290）；C# 本机未编译，家里机 PlayMode（fade-in / pop-in BackOut overshoot / 并发不同通道 / onComplete tag+prop / 颜色 / kill 停末值 / clear 回 CSS / 零回归 / 500 节点 stress）。subagent-driven 8 task 全 Approved + final review Ready（opus，跨语言 u32↔repr(u8) 三处对齐 + EventRecord 20B ABI 不变 + 零回归每读取点核实，无 Critical/Important）。spec `docs/superpowers/specs/2026-06-25-v1d.4-gtween-tween-design.md`、路线 `docs/roadmap/v1d-plan.md`。

> **v1d.5 实现状态（@ e8ef32c，待家里机验）**：ScrollPane + 滚动条 + 滚轮 + 手势仲裁落地（全 core `scroll.rs` 新 952 行 + `input.rs`/`transform.rs`/`render`/`hit`/`stage.rs`，机制镜像 fgui ScrollPane 已核实源码），**关 v1 验收 #2**，v1d 收尾。**overflow 扩展**（`ResolvedStyle.overflow_hidden:bool`→`overflow_x/y:OverflowMode` `#[repr(u8)]` Visible/Hidden/Scroll/Auto Default Visible 零回归；CSS `overflow` shorthand 设双轴 + `overflow-x/y` longhand 设单轴；任一轴≠Visible→`clip_rect=Some(border 框)` 复用现有 clip；pkg v6→**v7** bincode）+ **offset 虚拟平移注入 `compute_world_transforms` DFS**（`world[N]=world[父]∘T(-父.scroll_pos)∘local[N]` 父是滚动容器时；容器自身不含自己 offset/后代每层累积；复用 v1d.3 world matrix + 现有 clip → **blob v4/MirrorPool/shader 零改**；scroll_pos=0 no-op 零回归）+ **物理自维护 tween 不走 GTween**（`ScrollPaneState::{drag_follow/begin_inertia/begin_bounce/advance/apply_wheel/set_pos}`，cubic_out，fgui 常量；**|v| 非 v²** 坑 54；DECELERATION_RATE f64；PULL_RATIO=0.5 越界打折；回弹非弹簧 cubicOut 0.3s 回边界）+ **仲裁 per-slot**（TouchSlot +scroll_candidate/scrolling_pane 字段；scroll-vs-drag 阈值赛跑 **先达者赢**[scroll 8/20 > drag 2/10 但 drag 常先达] + 轴锁[V-only 让水平] + 嵌套[内层优先让出提升]；scroll-start cancel click）+ **滚轮**（`WheelEvent` `#[repr(C)]` 16B + `set_wheel_input` FFI + `apply_wheel_to_hit` hit→最近 effective；不产事件；新旧输入双路径）+ **合成 scrollbar**（`build_render_nodes` 追加 thumb RenderNode sentinel `node_id=container|V/H_THUMB_FLAG`[0x4000_0000/0x2000_0000] + `hit_scrollbar_grip` 前置 + grip 拖拽；坑 55 sentinel 跨链解码）+ **tick 重排**（compute_world_transforms 从 solve 后移到 process/keys 后——读 scroll_pos 同帧进 world matrix 零拖拽延迟；hit_test 1 帧差 §8.2 认；首帧 guard 防 OOB；**偏离 §12.7(2b') "在 style/layout 前"**[需 content_size + 拖拽事件驱动]，见 §12.7/§15 amendment）。version v1d.5；**2 新常驻 FFI**（`set_wheel_input`/`set_scroll_pos`）+ 手补 `LoomGUIWheelEvent.cs` 镜像（坑 35）；**pkg v7 / blob 保持 v4**；.dll 重编（1739776→1755136B）。**两机约束**：core+ffi cargo test 全绿（core 318+ffi 40+snapshot 3）；C# 本机未编译，家里机 PlayMode 9 点（拖拽跟手/惯性/回弹/滚轮/grip/auto·scroll 显条/嵌套轴锁/scroll-vs-draggable/SetScrollPos+零回归+500 stress）。subagent-driven 12 task 全 Approved + final review Ready（opus，跨任务 seam offset↔仲裁↔tick↔sentinel↔render/hit 逐链 sound，无 Critical/Important）。spec `docs/superpowers/specs/2026-06-26-v1d.5-scrollpane-design.md`、路线 `docs/roadmap/v1d-plan.md`。踩坑：fgui v2 变量名误导 v²→|v|（坑 54）、sentinel id 跨链致 apply_wheel OOB（坑 55）。**v1d 全收尾，v1 ship-ready（#2 家里机验过即 ship）**。
...
- **惯性回弹物理**：**不走 GTween**（content 异步变化时 GTween 的固定 end 会跳变）。ScrollPane 自维护可变 target 的 tween，content size 变化时按状态补偿 start、不突变。tick 时机 = §15 (2b')。**v1d.5 实现偏离原 (2b') "紧随 GTween、在 style/layout 前"**：scroll 物理需 content_size（solve 产物）且拖拽跟手是事件驱动（须在 process），故放 **solve 后、process 后、compute_world_transforms 前**——满足 (2b') 真意"本帧 scroll 偏移进 transform 与命中"（drag+inertia+wheel 同帧进 world matrix，零拖拽延迟；process 的 hit_test 用上帧 world_transforms，1 帧差可接受，仲裁在 Down 未滚动前判定）。**禁止 GTween 直接 tween `scroll_pos`**（API 层挡，避免双写）。
