...
- **设计文档 vs skill**：`docs/design/main-design.md`（设计契约/当前实现真相）、`docs/design/fence.md`（围栏）、`docs/roadmap/roadmap.md`（范围+机制草稿）、`docs/superpowers/specs|plans/`（历史 per-feature 记录）。`.claude/skills/knowledge-reference` skill 是实操知识库（踩坑/依赖 API/调试技巧/已知问题）——开工前读它，查"具体怎么干 + 坑在哪"。
